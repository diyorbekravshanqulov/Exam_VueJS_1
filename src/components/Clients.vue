  ...
  <div class="temp flex justify-center">
    <div class="container max-md:px-[5%] md:grid md:grid-cols-3 gap-[10%] md:mt-[57px] md:py-[128px] py-[64px]">
      <div class="md:col-span-1 col-span-2">
        ...
        <p class="md:mt-4 max-md:text-[14px] mt-8" style="color: rgba(40, 41, 56, 1)">
        ...
        <div class="swiper-container overflow-hidden">
          <div class="swiper-wrapper">
            <div class="swiper-slide" v-for="(testimonial, index) in testimonials" :key="index">
              <h5 class="md:text-[32px] text-[20px] font-medium md:leading-[48px] leading-[36px]" style="color: rgba(40, 41, 56, 1)">
                "{{ testimonial.text }}"
              </h5>
              <div class="mt-12 w-full justify-between flex">
                <div class="flex gap-[14px]">
                  <img :src="testimonial.img" alt="testimonial.name" />
                  <div class="flex flex-col justify-start items-center gap-2 max-md:-1">
                    <h6 class=" text-[14px] md:text-[18px]" style="color: rgba(40, 41, 56, 1)">
                      {{ testimonial.name }}
                    </h6>
                    <p class="text-[10px] md:text-[12px]" style="color: rgba(40, 41, 56, 1)">
                      {{ testimonial.position }}
                    </p>
                  </div>
                </div>
                <div class="flex items-center gap-[10px]">
                  <button @click="goToPrevSlide" class="text-[#282938] md:w-[48px] md:h-[48px] h-[34px] w-[34px] flex items-center justify-center rounded-full hover:bg-[#5239FA] hover:text-white text-xl md:text-4xl">
                    &lt;
                  </button>
                  <button @click="goToNextSlide" class="md:w-[48px] md:h-[48px] h-[34px] w-[34px] flex items-center justify-center rounded-full text-xl md:text-4xl hover:bg-[#5239FA] hover:text-white ">
                    &gt;
                  </button>
                </div>
              </div>
